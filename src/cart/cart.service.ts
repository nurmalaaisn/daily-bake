...
export class CartService {
    ...
    async addToCart(userId: number, dto: AddToCartDto) {
        ...
        try {
            ...
            throw new InternalServerErrorException('Gagal memasukkan produk ke keranjang');
        }
    }
    ...
    async getCart(userId: number) {
        ...
        if (!cart || cart.cartItems.length === 0) {
            return { message: 'Keranjang belanja kamu kosong', data: [], summary: { totalItems: 0, totalPrice: 0 } };
        }
    ...
    async removeCartItem(userId: number, productId: number) {
        ...
        if (!item) throw new NotFoundException('Produk tidak ditemukan di dalam keranjang');
